...
| 항목 | 내용 |
| --- | --- |
| 설명 | 내부 위험 주소 DB 또는 KYT Provider 조회 결과 HIGH/CRITICAL 등 고위험으로 분류된 지갑으로 출금 |
| 점수 | 100 |
| blocking | true |
| 위험 의미 | 해킹 자금, 제재 주소, 피싱 주소, 믹서 연관 주소 가능성 |
| 조치 | 즉시 `BLOCKED` 및 심사 Case 생성 |

#### KYT Provider Mock 연동

`HIGH_RISK_WALLET` Rule은 `RiskContextBuilder`가 구성한 지갑 위험도 스냅샷을 사용합니다.

지갑 위험도 조회 흐름은 다음과 같습니다.

```text
WalletRiskService.getWalletRisk(chainType, address)
  -> Redis cache 조회
  -> cache miss 시 wallet_address_risk DB 조회
  -> DB miss 시 KytLookupService.lookupAndSync(...)
  -> MockKytProvider.lookup(...)
```

Mock Provider는 주소 패턴에 따라 KYT 위험도를 반환합니다.

`riskCategory`는 `KytRiskCategory` enum 값으로 관리됩니다.

| Address Pattern | WalletRiskLevel | RiskCategory | RiskScore | DB 저장 |
| --- | --- | --- | ---: | --- |
| `SANCTION` | `CRITICAL` | `SANCTIONED_ADDRESS` | 100 | 저장 |
| `HACKED` | `HIGH` | `HACKED_FUNDS` | 100 | 저장 |
| `MIXER` | `HIGH` | `MIXER` | 90 | 저장 |
| `PHISH` | `HIGH` | `PHISHING` | 85 | 저장 |
| 기타 | `LOW` | `NORMAL` | 0 | 저장하지 않음 |

위험 주소는 `wallet_address_risk`에 동기화하여 이후 조회에서 재사용합니다. 정상 주소는 DB에는 저장하지 않고 캐시 결과로만 재사용합니다.
